...
const handleShareReport = useCallback(async () => {
  ...
  const topThreats = scanResult.profiles
    .slice(0, 3)
    .map((profile) => {
      const threat =
        profile.threatPatterns?.[0]?.description ||
        profile.suspicionFactors?.[0]?.description ||
        "Potential privacy risk detected";

      return `${profile.appName}: ${threat}`;
    })
    .join("\n");

  const report = `OMNYX Privacy Report

Scanned: ${scanResult.scannedAt.toLocaleString()}
Apps analyzed: ${scanResult.appCount}
Privacy score: ${privacyScore.current}/100

Risk breakdown:
Critical: ${scanResult.criticalCount}
High: ${scanResult.highRiskCount}
Medium: ${scanResult.mediumRiskCount}
Safe: ${scanResult.safeCount}
...
Generated by OMNYX`;
  ...
  try {
    ...
    Alert.alert("Error", "Unable to share the report at this time.");
  }
  ...
  <TouchableOpacity
    ...
    style={{
      ...
      paddingVertical: 7,
      borderRadius: 10,
      backgroundColor: `${C.safe}12`,
      borderWidth: 1,
      borderColor: `${C.safe}30`,
    }}
  >
    <Text
      style={{
